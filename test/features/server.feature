...
Feature: Server
  ...
  Scenario: Existing cfg with HTTP
    ...
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with HTTP
    Then I should receive a valid cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from HTTP

  Scenario: Existing cfg twice with HTTP
    Given I have a valid vcs token
    And I start nonnative
    And I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with HTTP
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with HTTP
    Then I should receive a valid cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from HTTP
  ...
  Scenario Outline: Missing cfg with HTTP
    ...
    Examples:
      | app     | ver    | env     | cmd     |
      | missing | v1.0.0 | staging | server  |
      | test    | v1.0.0 | staging | missing |

  Scenario: Misconfigured cfg with HTTP
    Given I have a misconfigured vcs token
    And I start nonnative
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with HTTP
    Then I should receive a missing cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from HTTP
  ...
  Scenario: Existing cfg with gRPC
    Given I have a valid vcs token
    And I start nonnative
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with gRPC
    Then I should receive a valid cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from gRPC

  Scenario: Existing cfg twice with gRPC
    Given I have a valid vcs token
    And I start nonnative
    And I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with gRPC
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with gRPC
    Then I should receive a valid cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from gRPC
  ...
  Scenario: Misconfigured cfg with gRPC
    ...
    And I start nonnative
    When I request "test" app with "v1.0.0" ver from "staging" env and "server" cmd with gRPC
    Then I should receive a missing cfg from "test" app with "v1.0.0" ver and "staging" env and "server" cmd from gRPC
