...
Given('I have a {string} valid setup') do |source|
  ...
  when 's3'
    files = [
      ['test/v1.6.0/staging/server.config.yml', '.config/test/v1.6.0/staging/server.config.yml'],
      ['test/v1.6.0/staging/eu/server.config.yml', '.config/test/v1.6.0/staging/eu/server.config.yml']
    ]
...
Given('I have a {string} invalid setup') do |source|
  ...
  when 's3'
    files = [
      'test/v1.6.0/staging/server.config.yml',
      'test/v1.6.0/staging/eu/server.config.yml'
    ]
